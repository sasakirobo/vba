...
public static class DuckDbHelper
{
    ...
    private static object[,] ReadResultsWithLimit(DuckDBDataReader reader, int maxRows, out bool limitReached)
    {
        ...
        foreach (var _ in reader)
        {
            ...
            for (var i = 0; i < fc; i++)
            {
                ...
                try
                {
                    ...
                    {
                        // ── BigInteger / ulong ──────────────────────────
                        ColKind.BigInt => ReadBigInt(reader, i),

                        // ── BLOB ────────────────────────────────────────
                        ColKind.Blob => ReadBlob(reader, i),

                        // ── decimal ─────────────────────────────────────
                        ColKind.Decimal => decimal.ToDouble(reader.GetDecimal(i)),

                        // ── float ────────────────────────────────────────
                        ColKind.Float => (object)ToExcelValue((double)reader.GetFloat(i)),

                        // ── TimeSpan / DuckDBTimeOnly ────────────────────
                        ColKind.TimeSpan => ReadTimeSpan(reader, i),

                        // ── DateTimeOffset ───────────────────────────────
                        ColKind.TimeTz => ReadTimeTz(reader, i),

                        // ── DateOnly ─────────────────────────────────────
                        ColKind.DateOnly => reader.GetDateTime(i),

                        // ── Guid ─────────────────────────────────────────
                        ColKind.Uuid => reader.GetGuid(i).ToString(),
                ...
                {
                    // 個別セルの変換失敗は ExcelErrorValue で表現し、処理を継続する
                    Debug.WriteLine(
                        $"[xlDuckDb] 列[{i}] ({reader.GetName(i)}) 変換エラー: {ex.GetType().Name}: {ex.Message}");
                    row[i] = ExcelError.ExcelErrorValue;
                }

                // 変換後の値に対して NaN / Inf チェックを通す（Decimal→double 変換後等）
                row[i] = row[i] switch
                {
        ...
        // maxRows ぴったりで終わった場合も limitReached を確認
        if (!limitReached)
