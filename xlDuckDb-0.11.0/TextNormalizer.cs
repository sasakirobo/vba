...
{
    // ── ICU Transliterator（半角カナ → 全角カナ専用）────────────────────
        ...
        string? additionalReplace = null)
    {
        ...
        {
            var t = Normalizer.Normalize(s, Normalizer.UNormalizationMode.UNORM_NFKC);
            t = UChar.FoldCase(t, true);
            AddStep("After NFKC+CaseFold", t);
        }
        ...
        string? additionalReplace)
    {
        ...
        if (options.HasFlag(NormalizeOptions.NfkcCaseFold))
        {
            s = Normalizer.Normalize(s, Normalizer.UNormalizationMode.UNORM_NFKC);
            s = UChar.FoldCase(s, true); // full Unicode case fold
        }
